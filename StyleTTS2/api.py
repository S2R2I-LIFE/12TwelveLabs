...
# Fine-tuned voice synthesizers loaded at runtime; keyed by voiceId
CUSTOM_SYNTHESIZERS: dict = {}
...
async def lifespan(app: FastAPI):
    ...
    # Re-register any custom voices that were deployed before this restart
    ...
    if os.path.isdir(voices_dir):
        for voice_id in os.listdir(voices_dir):
            ...
            checkpoint = os.path.join(voice_dir, "checkpoint.pth")
            reference = os.path.join(voice_dir, "reference.wav")
            if os.path.isfile(checkpoint) and os.path.isfile(reference):
                try:
                    ft_synth = StyleTTS2Inference(
                        config_path=os.getenv("CONFIG_PATH", "Models/LibriTTS/config.yml"),
                        model_path=checkpoint,
                    )
                    CUSTOM_SYNTHESIZERS[voice_id] = ft_synth
                    TARGET_VOICES[voice_id] = reference
                    logger.info(f"Re-registered custom voice: {voice_id}")
                except Exception as e:
                    logger.error(f"Failed to load custom voice {voice_id}: {e}")
...
async def generate_speech(request: TextOnlyRequest, background_tasks: BackgroundTasks):
    ...
    try:
        ...
        # Use fine-tuned synthesizer if one was registered for this voice
        active_synthesizer = CUSTOM_SYNTHESIZERS.get(request.target_voice, synthesizer)

        # Compute style for requested voice
        current_style = active_synthesizer.compute_style(ref_audio_path)
        logger.info(
        ...
        for i, chunk in enumerate(text_chunks):
            ...
            audio_chunk = active_synthesizer.inference(
                text=chunk,
                ref_s=current_style
            )
...
@app.post("/admin/register-voice", dependencies=[Depends(verify_api_key)])
async def register_voice(req: RegisterVoiceRequest):
    """Hot-register a fine-tuned voice without restarting the container."""
    if not os.path.isfile(req.checkpointPath):
        raise HTTPException(status_code=400, detail=f"Checkpoint not found: {req.checkpointPath}")
    if not os.path.isfile(req.referenceAudioPath):
        raise HTTPException(status_code=400, detail=f"Reference audio not found: {req.referenceAudioPath}")

    try:
        ft_synth = StyleTTS2Inference(
            config_path=os.getenv("CONFIG_PATH", "Models/LibriTTS/config.yml"),
            model_path=req.checkpointPath,
        )
    except Exception as e:
        logger.error(f"Failed to load checkpoint for {req.voiceId}: {e}")
        raise HTTPException(status_code=500, detail=f"Failed to load checkpoint: {e}")

    CUSTOM_SYNTHESIZERS[req.voiceId] = ft_synth
    TARGET_VOICES[req.voiceId] = req.referenceAudioPath
    logger.info(f"Registered custom voice: {req.voiceId} ({req.voiceName})")
    return {"registered": True, "voiceId": req.voiceId, "voiceName": req.voiceName}
